...
| `MAILAGENT_CUSTOM_AGENTS_ENABLED` | `false` | Agent 开放性 epic **S4**（task 07-02）custom agent 内核：`report_agent` 泛化 type='custom'（**DB v30** trigger_json/tool_policy_json/budget_json），cron(croniter UTC marker)/email_filter(new_watcher 第 5 hook) 触发 → async_jobs(job_type='agent_run' 两族分区) → AgentRunWorker poke → gateway `POST /api/ai/agent-run` headless 跑路径 C（spec 回拉 CAS one-shot + contextMode 从 trigger.kind 派生 + 矩阵地板 + allowed_tools 交集只减不加 + budget 三维 + session origin='agent' **CHAT_DB v19**）；headless 写恒 HITL（岛）或作废，**`status=succeeded && outcome='paused_handoff'` 永不渲染为成功**（读态唯一入口 `src/agents/run_state.py::derive_agent_run_state`）；app 关=全停，cron 重启单次 catch-up。off → hook/worker/端点全灭，ToolSet/catalog 字节级不变（S4 零新 gateway 工具）。详见 ai-sdk-gateway-architecture.md §13.19 |
...
| `src/agents/` | Custom agent 内核（S4，flag `MAILAGENT_CUSTOM_AGENTS_ENABLED` 默认 off）：`trigger`(trigger_json 判别式解析+保存时深校验) / `matcher`(AgentEmailMatcher 邮件事件匹配) / `trigger_worker`(cron croniter UTC marker) / `email_dispatch`+`run_queue`(第 5 hook 分发+enqueue 幂等/runs-day 门) / `run_worker`(AgentRunWorker claim→poke gateway→终态回写) / `fence`(UNTRUSTED_EMAIL_BODY envelope 围栏) / `run_state`(paused_handoff 读态唯一入口)。见 ai-sdk-gateway-architecture.md §13.19 |
