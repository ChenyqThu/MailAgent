...
| **MCP Connectors**（外部服务工具面：`agent_config.db` 双表 + OAuth 2.1/PKCE/DCR + needs_reauth/orphan 状态机 + 四调用方授权[manual 读免批写恒 HITL / headless `grant_connectors` / 预处理 read 硬天花板] + delete 恒不注册 + 灰度 flag） | 动 `src/connectors/` / connector 端点 / gateway connector 工具注入 / 第七能力卡 / `MAILAGENT_MCP_CONNECTORS` 前 | [`llm-agent/mcp-connectors.md`](./docs/reference/llm-agent/mcp-connectors.md) |
