...
| **资料库 Library**（受管多根文件树 + 独立 `library.db` + 一组 agent 文件工具） | 动 `src/library/` / `library.db` / `/api/library` / 资料库前端 / `libpreview://` 预览 / 七个 `library_*` 工具前 | [`library/library-architecture.md`](./docs/reference/library/library-architecture.md) |
...
- EWS **2026-10-01 起默认阻断**、2027-04-01 完全退役。本机跑的是 DavMail **6.7.0**（EWS 模式）；上游 **6.8.0/6.8.1 已发版并带正式 Graph backend**，且 Outlook clientId 伪装在 Graph 下**仍可用、不需要 IT 审批**（`davmail.mode=O365Graph` + `davmail.enableOidc=false`）。**2026-08-26 复查后仍等待，两周后（2026-09 上旬）再看** —— 上游仍无 6.9，但 trunk 已修 #506（草稿 APPEND 400）、delta sync 默认开启；🔴 **卡点收敛为一行未修的 bug**：`O365Token.java:255` refresh 时写死 EWS resource，1h 后 Graph 全线失效（8-14 上游新增的 token 检查还会**清空 refresh token**）。⇒ **复查判据不是「有没有 6.9」而是「那一行修没修」**；未修则仍需自建 trunk + 一行 patch，换版本解决不了。迁移不需重灌数据（imapUid 同为 MAPI `0x0e23`、UIDVALIDITY 恒 1），但**必须重认证且回切也要重认证**；且 6.8 起 `mode`/`authentication` 拆键，**不改配置不会切到 Graph**（旧值走 legacy fallback 仍是 EWS），改 mode 后须显式补 `davmail.authentication=O365Manual`。完整实证与复查判据见 [`docs/reference/architecture/roadmap-post-cutover.md`](./docs/reference/architecture/roadmap-post-cutover.md) §5.1。
...
| `src/matters/` | Matters/事项域（17 文件，`service.py` 独占 ~4600 行）：域服务单写面 / repository / run spec 与 worker / trigger envelope / 关注信号 / 资料身份归一 / 时间线 / **行动项派发执行契约（v71 `matter_item_dispatch` 七值状态机恒服务端 CAS，`matter_item_run` + `matter_item_report`，详见 matters-architecture.md §4.3）**。🔴 **三条入口安全姿态不同，不许合成一条路径**（创建带调研=纯读端点 / 定时跟进=headless run / 事项对话=交互式）。🔴 跟进 run 的 `allowedTools` 恒 `[]`、`grantExec` 永不写。🔴 v52 contact 索引独立常量**不进** `MATTER_INDEX_DDLS`（老库重放会炸）。🔴 时间线 `ON DELETE CASCADE`——永久删除的审计只能落日志 |
...
| `src/library/` | 资料库域：`db`（`library.db` 幂等 DDL + **独立版本梯，不进 `DB_VERSION`**）/ `paths`（多根 jail）/ `repository` / `service`（唯一写面）/ `extract`（复用 `attachment_text.extract_text`）/ `embed`（语义向量）/ `constants`（零依赖叶子，TS 镜像有 parity 闸）/ `resource_resolver`（给 matters 的存在性回调）。🔴 **文件系统是正文 SSoT，索引是元数据投影 + id 分配器**，`(id ↔ rel_path)` 永不重算、文件消失标 `missing` 不删行（跨模块引用 `library:{id}` 不悬空）。🔴 写侧强制在服务端 `_assert_writable`（投影区/`.trash` 拒写、custom agent 只有 `agent-docs/`）；无人值守免卡只认 `agent-docs/`，`library_move`/`library_delete` 永不进免卡通道 |
