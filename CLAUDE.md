...
| **Agent 体验大版本专项**（master plan，P0-P4 有序门控：eval 网固化 → memory/skill 内核 → assistant-ui/AI SDK 换引擎；整合 06-22 剩余阶段 + chat-panel，接力非竞争） | 接续 06-22 剩余阶段(memory/skill) / 推进 chat-panel 换引擎 / 跑 agent 回归网前 | [`agent-experience-epic/README.md`](./docs/plans/agent-experience-epic/README.md)（hub）+ [`chat-panel-...`](./docs/plans/chat-panel-ai-sdk-assistant-ui-refactor/) + 回归网 `tests/agent_eval/` |
