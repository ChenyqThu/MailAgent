...
| `MAILAGENT_CUSTOM_AGENTS_ENABLED` | `true` | Custom AI Agents（**默认开，E3 cutover 2026-07-06**；env 显式 false 应急回退=hook/worker/端点/CRUD 工具全灭字节级回 S4 前；**on 不配 grant/规则=恒 HITL**，per-agent 仍需 `report_agent.enabled` + `type='custom'`）。S4 内核=trigger(cron/email_filter)→async_jobs→AgentRunWorker→gateway headless 路径 C（DB v30、CHAT_DB v19；`paused_handoff` 永不渲染为成功，读态唯一入口 `src/agents/run_state.py`）；S5 产品化=Settings 全字段 CRUD + 对话式六工具（恒人审）+ per-agent 独立白名单（DB v31）；S6=run 反查 + 审批红点链 + per-agent web/skill grant。exec 免卡叠加依赖 `MAILAGENT_OPENNESS_EXEC_TOOLS`。详见 ai-sdk-gateway-architecture.md §13.19-13.21 + [决策表](docs/plans/architecture-review-2026-07/e3-defaults-decision-table.md) |
...
| `src/reports/` | 报告 Agent 系统（日/周/月报）：`models`(ReportDoc 块模型) / `data`(取数+分组) / `summarizer`(LLM tool_use) / `assembler`(防幻觉权威回填) / `worker`(tick_loop 定时；fire 判定走共享求值器 `src/agents/schedule_rule`) / `store`(report_agent+report 表；`cadence_of` 在新排程形状下从 `rule.freq` 派生，`agent_with_cadence_override` = manual-run `--cadence` 唯一入口)。见 [`docs/reference/remote-chat-report/report-agent-prd.md`](./docs/reference/remote-chat-report/report-agent-prd.md) |
| `src/agents/` | Custom agent 内核 + 产品化（S4+S5，flag `MAILAGENT_CUSTOM_AGENTS_ENABLED` 默认开，见上方开关表）：`trigger`(trigger_json 三态判别式 cron\|schedule\|email_filter 解析+保存时深校验+**S5 `parse_tool_policy` typed 严格化 grant_exec**) / **`schedule_rule`**(结构化排程 occurrence 求值器，**与 `src/reports/worker` 共用、禁止各写一份**；契约见 [`architecture/schedule-rule-contract.md`](./docs/reference/architecture/schedule-rule-contract.md)) / `matcher`(AgentEmailMatcher 邮件事件匹配) / `trigger_worker`(定时族 = cron croniter + schedule 求值器，UTC marker + 配置 hash 失效) / `email_dispatch`+`run_queue`(第 5 hook 分发+enqueue 幂等/runs-day 门) / `run_worker`(AgentRunWorker claim→poke gateway→终态回写+**S5 岛结果通知 completed/error**) / `fence`(UNTRUSTED_EMAIL_BODY envelope 围栏) / `run_state`(paused_handoff 读态唯一入口，8 值域)。**S5 产品面**分散于 `src/api/routers/agent_runs.py`(run 历史 `GET /api/agent-runs` + tool-options 端点 + `DEFAULT_CUSTOM_AGENT_ALLOWED_TOOLS` 默认安全集投影 + 免卡 badge 投影) · `src/api/routers/agent.py`(per-agent 规则 CRUD `/policy/rules` + `/skills/entrypoints`) · `src/agent_config/policy.py`+`store.py`(domain_write capability + `context_mode+agent_id` 双键物理隔离 + pinned-entrypoint 形状闸) · gateway `tools/agents.ts`(对话式 CRUD 六工具)+`tools/policy.ts`(`AgentModeGrants` 矩阵第三参)+`tools/write.ts`(headless-only policyEvaluate 注入)。**S6** 加执行记录反查（`agent_runs.py` state 过滤/pending-count + gateway `/api/ai/approval/pending` 真值端点）+ per-agent web grant/skill 挂载（`policy.py` `WebMatcher{origin}` + canonical `_normalize_origin` 单源 + `web.py` redirect 聚合集[candidate ∪ 首跳]，gateway `tools/web.ts` 三档免卡）+ custom-agent A2UI 审批卡（`a2ui.ts` before/after diff）。见 ai-sdk-gateway-architecture.md §13.19（S4）+ §13.20（S5）+ §13.21（S6） |
