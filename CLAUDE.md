...
| `MAILAGENT_LLM_PROVIDER_REGISTRY` | `false` | LLM 上游多 Provider 体系（task 07-12，灰度中）：on = provider/model 配置权威切到 `agent_config.db` 双表（`llm_provider`/`llm_model`，key Fernet 加密），gateway 走 AI SDK `createProviderRegistry`、Python 分类/报告/mem0 走 protocol 路由、translate/⌘K NL 搜索同源、Settings 显示「模型服务」区；模型引用统一 `providerId:modelId`（无冒号 legacy id → `default`）。首次启动表空时 `LLM_API_BASE/KEY/MODEL/ENABLED_MODELS` seed 成 default provider 行，行落地后**行权威**（env 键降级为首次 seed 默认）。off（默认）= 老 `createAnthropic` 单例 + 前缀路由字节级现状。双端读：Python pydantic（重启生效）+ Node main env（重启 app）。详见 llm-provider-registry.md |
