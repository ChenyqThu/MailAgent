...
| `MAILAGENT_CHAT_DETACHED_RUNS` | `true` | harness-chat 批（2026-07-15）：chat run 与客户端连接解耦（**默认开**；env 显式 false 应急回退=字节级回 close→abort 直连）。on = 切会话/关面板不中断流，gateway 自持 drain 到 onFinish 正常落库（ai@7 pipe 在 res 销毁后停止消费，故手工 drain、wire 字节级 parity 有测试 pin）；配 ActiveRunRegistry：`GET /api/ai/run/active`（「AI 仍在后台输出」真值）+ `POST /api/ai/run/stop`（composer 停止钮显式停止）+ 同 session 并发第二 POST 409（审批 renderer-resume 二次 POST 不受影响，暂停即释放 registry，有 pin 测试）。落库后广播 `chat:turn-persisted`（'finished'\|'paused'）驱动切回自动刷新与未读标记（CHAT_DB v20 `last_read_at`）。main-env-only 无 vite define。 |
