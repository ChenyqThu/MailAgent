...
| `LLM_PREPROCESS_CONTEXT_SOURCE` | `""`（继承） | 预处理分类 system prompt 的「参考上下文源」二选一（task 07-21，`_resolve_context_source` 单源）：`standing_docs`=注入身份文档（沿用预处理行 `context_docs` 勾选）、不注入 notion；`notion_context`=注入 `LLM_CONTEXT_PAGE_ID` 页面、不注入身份文档；**二者互斥不叠加**（旧版是双注入，owner 反馈冗余）。空/非法=升级继承：配了 `LLM_CONTEXT_PAGE_ID`→`notion_context`，否则 `standing_docs`（老库零迁移，无 DB 列）。pydantic 冻结单例，改值需重启后端。编辑入口=前端「Custom AI → AI 邮件预处理」抽屉（设置-AI 的 Prompt/context page 区已移除）。配套：`LLM_CONTEXT_PAGE_ID` 语义收窄为「仅 `notion_context` 时生效，**不再注入 chat**」（chat 由 Standing Context 单源）。 |
