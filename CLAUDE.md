...
| **Agent 体验大版本专项**（P0-P4 换引擎 **✅ cutover v0.20.0**；mem0 记忆核心重构 **M1-M5 ✅ + web→ai-sdk(任务A) ✅ + Settings 身份文档编辑器 ✅**（M1-M4 随 **v1.0.1**；M5b[agent_memory_kv 退役] + 4 flag cutover 默认 ON 随 **v1.1.0**）；下一步 = 任务B[删 legacy harness]；P0-P4 过程档已归 `docs/archive/2026-06/`） | 推进 enhance 收尾 / web→ai-sdk / 跑 agent 回归网前 | [`agent-experience-epic/memory-skill-core-refactor.md`](./docs/plans/agent-experience-epic/memory-skill-core-refactor.md)（下一 epic 计划，含架构 review）+ [`next-phase-backlog.md`](./docs/plans/agent-experience-epic/next-phase-backlog.md) + 引擎架构 [`llm-agent/ai-sdk-gateway-architecture.md`](./docs/reference/llm-agent/ai-sdk-gateway-architecture.md) + 回归网 `tests/agent_eval/` |
