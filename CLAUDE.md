...
| `src/reports/` | 报告 Agent 系统（日/周/月 + custom）：`models` 定义 18 种 ReportDoc block 的 Python canonical vocabulary，TS 镜像有跨语言抽取闸；`data`/`summarizer`/`assembler` 负责取数、LLM tool_use 与防幻觉回填；`worker` tick 走共享 `src/agents/schedule_rule`；`store` 管 report_agent+report，custom cadence 支持同日多份与 replace 稳定归宿。gateway `report_write` 是 artifact class，本地产物免审批；Python 边界再次验证块类型/数量与 image 内部资源限制。见 [`docs/reference/remote-chat-report/report-agent-prd.md`](./docs/reference/remote-chat-report/report-agent-prd.md) |
| `src/agents/` | Custom agent 内核 + 产品化（flag `MAILAGENT_CUSTOM_AGENTS_ENABLED` 默认开）：`trigger` 解析 cron\|schedule\|email_filter、两项预算（runs/day + runtime；legacy `max_steps` 忽略）与 tool policy；`schedule_rule` 与 report worker 共用；`email_dispatch`+`run_queue` 负责幂等 enqueue、runs/day 门及可见 `skipped` 行；`run_worker` claim→gateway→终态；`run_state` 是 paused_handoff/expired/skipped 的 9 值读态唯一入口，并投影 steps/tokens/duration。产品面包括六能力卡→canonical 工具/grant 映射、CHAT_DB v21 interactive/agent 会话隔离与 pin/star、Oreo `avatar_json` 身份、审批反查、per-agent web/skill grant，以及只注入 advertised code-owned `custom_agent` builtin skill 的对话式创建流程；exec/outbound/capability_change 安全地板不变。见 ai-sdk-gateway-architecture.md §13.19-13.21 |
