...
| `KOS_REQUIRE_LABELED` | `false` | issue #49：「AI 从未标注优先级」是独立于优先级枚举的第三态。默认 false = `_normalize_priority(None)` 把它隐式并入 `normal`，于是 `KOS_INGEST_PRIORITY_FLOOR=normal` 会把从未跑过 LLM 的历史邮件**全部放行**（实测占候选量 ~89%）。true = 未标注直接跳过；增量 producer（`push_email_to_kos`）与 `bulk_ingest` 两处过滤点共用 `passes_priority_gate()` 单源，且与 floor **独立**（`floor='low'` 也照挡）。bulk 侧 `--require-labeled` 可覆盖 env。`priority_at_or_above` 的 None→normal 老语义**有意不动**（改返回值波及所有调用点且不向后兼容） |
