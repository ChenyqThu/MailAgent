...
| **Agent 体验大版本专项**（P0-P4 换引擎 **✅ cutover v0.20.0**；下一步 = 核心调度层重构 M1-M4[memory/skill] + web→ai-sdk dogfood；P0-P4 过程档已归 `docs/archive/2026-06/`） | 推进 enhance M1-M4 / web→ai-sdk / 跑 agent 回归网前 | [`agent-experience-epic/memory-skill-core-refactor.md`](./docs/plans/agent-experience-epic/memory-skill-core-refactor.md)（下一 epic 计划，含架构 review）+ [`next-phase-backlog.md`](./docs/plans/agent-experience-epic/next-phase-backlog.md) + 引擎架构 [`llm-agent/ai-sdk-gateway-architecture.md`](./docs/reference/llm-agent/ai-sdk-gateway-architecture.md) + 回归网 `tests/agent_eval/` |
