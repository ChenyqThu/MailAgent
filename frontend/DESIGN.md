...
- **Bot avatar loop motion** (`shared/bot-avatar/`, 2026-08-13): the state-driven blink/expression/gaze loop is a sanctioned duration-tier exemption (like Strands) — spring morph is engine-internal, NOT the GSAP spring whitelist's concern. Static tier is the default everywhere; `animated` exists at exactly 3 sites (TurnPresence 28px, panel header 20px, editor preview 48px) — adding an animated site requires a perf review. Reduced-motion short-circuits in JS to the static tier. See `docs/bot-avatar.md`.
