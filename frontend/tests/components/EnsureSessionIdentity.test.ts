...
//
// 它有两个调用方：transport 的首次发送、以及事项对话切检索范围时的「审计先行」。两者之间用户
// 完全可能已经点到**另一件事**或换了会话。旧实现的去重只有一个「有没有在途」的 ref，于是事项 A
// 的在途创建会被事项 B 的调用复用，产出三种全都很坏的结局：
//   ① `recordChatScope(B, …, sessionA.id)` —— B 的检索范围审计写进 A 的会话；
