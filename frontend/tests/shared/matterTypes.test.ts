...
describe('Matter TypeScript vocabulary', () => {
  test('pins every canonical array and member count', () => {
    expect(MATTER_STATUSES).toEqual([
      'inbox',
      'planned',
      'active',
      'waiting',
    ...
    expect(MATTER_ITEM_KINDS).toEqual([
      'action',
      'milestone',
      'decision',
      'blocker',
    ...
    // 🔴 与 MATTER_ITEM_KINDS 的 milestone / decision 同名不同物：那是工作对象，这是叙事节点。
    expect(MATTER_PROGRESS_KINDS).toEqual(['goal', 'milestone', 'progress', 'signal', 'decision'])
    ...
    expect(MATTER_CHANGE_KINDS).toHaveLength(6)
    ...
    expect(MATTER_SEARCH_FIELDS).toEqual([
      'title',
      'description',
      'current_summary',
      'status',
