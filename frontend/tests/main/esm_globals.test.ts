...
// ASSERTIONS over frontend/src/electron/** — bare `__dirname` / `__filename` /
// `require(` are banned; the ESM-safe single source is lib/esm-paths.ts
...
const ELECTRON_SRC = fileURLToPath(new URL('../../src/electron', import.meta.url))
...
describe('no bare CJS globals in ESM electron source (P5 startup regression pin)', () => {
  const files = walkSources(ELECTRON_SRC)
  ...
  it.each(BARE_GLOBALS)('no bare %s anywhere under src/electron/**', (name, pattern) => {
    const offenders: string[] = []
    for (const file of files) {
      const lines = readFileSync(file, 'utf8').split('\n')
      lines.forEach((line, i) => {
        if (pattern.test(line) && !isAllowedLine(line)) {
          offenders.push(`${relative(ELECTRON_SRC, file)}:${i + 1}: ${line.trim()}`)
        }
      })
    }
    expect(
  ...
  it('main/index.ts pins esm-paths into the entry chunk via a static import', () => {
    ...
    const source = readFileSync(join(ELECTRON_SRC, 'main/index.ts'), 'utf8')
