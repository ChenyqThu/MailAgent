// 灵动 bot 头像 v2 —— 3D 参数曲面采样层。
// 移植自 avatar-lab `src/features/avatar/surfaces.ts`（owner fork of
// smontlouis/bible-strong-avatar-lab，AGPL-3.0 上游 —— 出处与许可注记见
// frontend/docs/bot-avatar.md）。裁剪面：去掉 wireframe 采样（surfaceSampleAt/
// surfaceNormalAt/tangent 法线族）与 i18n label —— MailAgent 只消费
// 「头部轮廓采样 + 正面贴合采样」两条路径。
...
/** 每种原语的出厂尺寸（avatar-lab surfacePresets 逐字），shapes.ts 据此建形状词表 */
...
const signedPower = (value: number, exponent: number): number =>
...
const clampRoundness = (roundness: number | undefined): number =>
  Math.max(0, Math.min(2, roundness ?? 0))

const diamondExponent = (config: SurfaceConfig): number => 1 + clampRoundness(config.roundness) / 2

const MIN_CUBE_SURFACE_POWER = 0.04

const cubeExponent = (config: SurfaceConfig): number => {
  if (config.roundness <= 0) return Infinity
  // 隐式 superellipsoid 幂从「近平面方块」推向椭球
  const surfacePower =
...
const diamond = (config: SurfaceConfig, longitude: number, latitude: number): Point3 =>
  lpSurface(config, longitude, latitude, diamondExponent(config))
...
const morphProgress = (config: SurfaceConfig): number => clampRoundness(config.morphRoundness) / 2
...
const coneRounding = (config: SurfaceConfig): { tipFraction: number; baseFraction: number } => ({
...
const morphedCylinderProfileAt = (config: SurfaceConfig, progress: number): RadialProfile =>
...
const coneProfileAt = (config: SurfaceConfig, progress: number): RadialProfile => {
  ...
  if (baseFraction > 0 && clampedProgress < baseFraction) {
    ...
    return {
      radiusScale: cubic(1 - baseFraction, 1, 1 - baseFraction / 2, 1 - baseFraction, curveProgress),
...
const morphedConeProfileAt = (config: SurfaceConfig, progress: number): RadialProfile =>
  morphProfileToEllipsoid(config, progress, coneProfileAt(config, progress))

/** cursor（光标锥 + 柱身）复合形的布局参数 */
...
const signedMagnitude = (value: number, exponent: number): number =>
  Math.sign(value) * Math.abs(value) ** exponent
...
/** 规范脸面坐标 (x, y) → 该原语正面曲面上的点 + 法线（眼睛贴合的核心） */
export const surfaceFrontSampleAt = (config: SurfaceConfig, x: number, y: number): SurfaceSample => {
  const radiusX = config.width / 2 || 1
  const radiusY = config.height / 2 || 1
  const radiusZ = config.depth / 2 || 1
  ...
  switch (config.type) {
    ...
    case 'cursor': {
      ...
      const sample = radialProfileFrontSample(bodyConfig, x, y - layout.bodyCenterY, cylinderProfileAt, 1)
      return {
