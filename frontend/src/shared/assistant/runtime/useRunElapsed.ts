// WP-14 — 回合级秒表（渲染器墙钟）。0813 轮 5 起唯一消费者是**后台 run 的在场行**
// （`TurnPresence.tsx::BackgroundRunPresence`）；它原来的载体（composer 上方的运行状态条）已随
// 那一轮整条退役，实时叙述搬进消息流的回合头像行。
//
// 与工具卡那口时钟（`tools/generic/useToolElapsed.ts`）的关系：同一套契约，不同的**起点来源**。
// 工具卡只能在自己挂载的那一刻起表；这口表还有第二种起点 —— detached（后台）run 的
// `GET /api/ai/run/active` 会给 `ageMs`（「这个 run 已经跑了多久」），换算成本地 epoch 起点后
// 传进来，秒表就能**接着**那个数往上走，而不是切走再切回时从 0:00 重新开始。这也正是两口表
// 有意不收敛成一个的原因（见 BackgroundRunPresence 的注释）。
//
// 沿用的三条契约（逐条对齐 useToolElapsed 的文件头）：
//   1. 没起点就不编数 —— 本 hook 只在「在场行可见」时被挂载（调用方把它放在只在进行中渲染的
...
// 把最终耗时冻住展示；后台在场行在 run 结束的那一刻整个消失，冻一个没人看的值只会让「上一个
// run 的读数在下一回合头几百毫秒闪一下」变成可能。
