...
export function ContextUsageRing(): React.JSX.Element | null {
  ...
  const open = openFor !== null && openFor === sessionId
  const setOpen = useCallback(
    (next: boolean | ((v: boolean) => boolean)) => {
      setOpenFor((prev) => {
        const wasOpen = prev !== null && prev === sessionId
        const want = typeof next === 'function' ? next(wasOpen) : next
        return want ? sessionId : null
      })
    },
    [sessionId]
  )
  ...
  return (
    ...
    <div className="mr-1.5" ref={rootRef}>
      ...
      {shouldRender && (
        ...
        >
          <ContextUsageDetails breakdown={breakdown} loading={promptLoading} />
        </div>
      )}
    </div>
  )
