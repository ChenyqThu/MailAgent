...
//
// 来由：TitleBar 右簇的浮层共用 `.theme-popover` 的几何，而那个类里的 `right: 12px`
// 只对簇内最右边的按钮成立 —— 铃铛在簇首，浮层因此整整偏出一个面板宽。同一个坑当年各修
// 各的修过三遍（Accent / Surface 各写一份「开弹时按按钮右缘算 right」，Theme 写死 88px
// 魔数，铃铛干脆没修），所以收敛到这里。那三枚 picker 已随 08-27 dogfood 修正批退役，
// 现存消费方只剩通知铃铛 —— 保留本 hook 是因为右簇再加浮层时同一个坑会原样复现。
//
