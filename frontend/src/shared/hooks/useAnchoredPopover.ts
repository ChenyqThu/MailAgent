...
//
// 来由：TitleBar 右簇的四个浮层共用 `.theme-popover` 的几何，而那个类里的 `right: 12px`
// 只对簇内最右边的按钮成立 —— 铃铛在簇首，浮层因此整整偏出一个面板宽。同一个坑已经各修
// 各的修过三遍（Accent / Surface 各写一份「开弹时按按钮右缘算 right」，Theme 写死 88px
// 魔数，铃铛干脆没修），第五个 picker 只会再犯一次，所以收敛到这里。
//
// 只做「右对齐触发器 + 视口收口」这一种形态：**纵向落点仍归 CSS**（`.theme-popover` 的
// top，四个浮层共用同一条水平线是 chrome 的既定观感），本 hook 不接管，只按浮层的实际
