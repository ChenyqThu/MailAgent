...
export interface ComposerModelOption {
  ...
  modelId: string
  /** 展示名：model 行的 displayName，缺省回落 modelId。 */
  displayName: string
  /** null = 上游未标注（不渲染 badge）；对象里只有显式 true 的位才算支持。 */
  capabilities: LlmModelCapabilities | null
  /** 该模型单次回答的最大输出 token 数（llm_model.max_output）；null = 未标注。 */
 ...
 *  退化成今天扁平列表那份信息（ref + 去前缀 id），不是空洞也不是谎。 */
export function buildComposerModelOption(
  ...
  return {
    ...
    modelId,
    displayName: model?.displayName?.trim() ? model.displayName.trim() : modelId,
