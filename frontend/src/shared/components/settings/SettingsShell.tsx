...
//
// Layout:
//   PageFrame (TitleBar + nav shell)
//     └── SettingsShell
//           └── content pane (760 max-w, glass-3 surface, overflow-y-auto)
//                 └── one <Tabs.Content> per tab
//
// 节导航**只有一处**：域二级栏（DomainPanel 的设置分支，词表/图标同源
// settingsTabMeta）。本组件不出任何 tab 触发面 —— Tabs root 纯粹由 URL 驱动
// （`value={tab}`），没有 TabsList/TabsTrigger。
//
// Active tab 来自 URL search param `?tab=general` (validateSearch 在
// router-instance.tsx 处理 enum + default). 切 tab 由 DomainPanel 的行写 search,
// 这样浏览器返回 / 刷新 / 深链都对得上.
//
...
export function SettingsShell(): React.ReactElement {
  ...
  return (
    <Tabs
      value={tab}
      // 🔴 不接 onValueChange：本组件不出触发面，切 tab 恒经域二级栏写 `?tab=`，
      // 再由上面的 `value={tab}` 单向读回来 —— 写 URL 是唯一入口，没有第二条腿。
