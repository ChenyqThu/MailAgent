...
/**
 * Parse a color channel string of the form "r g b" (0-255 each) into a [0-1, 0-1, 0-1] tuple.
 * This is the project-native token format (e.g. "248 138 125" for --c-accent).
 * Accepts either "r g b" (three space-separated integers/floats) or falls back to white.
 */
const parseChannel = (channel: string): [number, number, number] => {
  const parts = channel.trim().split(/\s+/)
  if (parts.length >= 3) {
...
const buildPalette = (colors: string[]): number[][] => {
  const filled = colors && colors.length ? colors : ['255 255 255']
  const padded: number[][] = []
  for (let i = 0; i < MAX_COLORS; i++) {
    const channel = filled[i] ?? filled[filled.length - 1]
    const [r, g, b] = parseChannel(channel)
    padded.push([r, g, b])
  }
  return padded
}
...
export default function Strands({
  ...
  // Keep propsRef in sync before the next animation frame (useLayoutEffect runs
  // synchronously after DOM mutations, before paint — safe for a mutable ref store).
  useLayoutEffect(() => {
    propsRef.current = {
      colors,
      count,
