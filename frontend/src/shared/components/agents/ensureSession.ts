...
//
// 懒建会话（第一次发送 / 事项检索范围的「审计先行」）是异步的，而它的两个调用方之间，用户完全
// 可能已经点到**另一件事**或换了会话。此前的去重只有一个「有没有在途」的 ref，于是事项 A 的在途
// 创建会被事项 B 的调用复用，产生三种全都很坏的结局：
//   ① `recordChatScope(B, …, sessionA.id)` —— B 的范围审计写进 A 的会话；
