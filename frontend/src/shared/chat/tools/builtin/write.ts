...
// All require user confirmation via ConfirmToolDialog before executing — flag/archive
// are tier=preview (reversible inside SQLite + outbox), draft is tier=edit (the user
// might edit the body before the draft is actually created). The confirmation dance
// lives in dispatch.ts; these handlers just do the write once dispatch invokes them.
//
...
//   - email_draft_reply           → platform.draftReply（electron: AppleScript createDraft,
//     host-local; http: throw E_NOT_IMPLEMENTED，fork 3 远程推迟）
...
/** Build the 3 write tools bound to the injected platform. */
export function createWriteTools(platform: ChatToolPlatform): ToolDef[] {
  ...
  const emailArchive: ToolDef = {
    ...
    handler: async (input, ctx): Promise<ToolResult> => {
      ...
      try {
        const data = (await platform.flagEmail(id, { processingStatus: '已完成' })) as FlagData
        return ok(
          {
            internal_id: id,
            archived: true,
  ...
  return [emailFlag, emailArchive, emailDraftReply]
}
