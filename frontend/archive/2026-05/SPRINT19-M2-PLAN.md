...
| PR-2f | ✅ ship 2026-05-23 | **L1 hot block 注入**：`slug.ts` + `sender_digest_cache.ts` (1h TTL + concurrent dedupe) + `harness.ts` 启动时 prefetch + `custom_api.ts` buildSystemBlocks 拆成 [stable + L1 hot block / session ctx] 双 block (cache_control 仅 stable). Gate `MAILAGENT_KOS_L1_HOT_BLOCK_ENABLED=true`. 跨邮件 chat session 都能命中 stable block cache (M1 时单 block 跨邮件必 miss). | ~800 (含 slug + cache + buildSystemBlocks 改造 + 28 新单测) |
| PR-2g | ✅ ship 2026-05-23 (待 user dogfood) | dogfood 包: `.env` 写好 7 个 KOS env (默认全 OFF); `scripts/dev/kos_smoke_test.sh` 4 步自动 verify; `docs/eval/m2-dogfood-checklist.md` layer-by-layer 验收清单; `docs/eval/email_scenarios.md` 加 S21-S25 共 5 个 KOS scenario. **翻 default flag 由 user 跑完 dogfood 后决定**. | ~600 (含 dogfood doc + smoke script + 5 scenario + .env config) |
