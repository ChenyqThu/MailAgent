...
- **表情 = 15 个数值参数**（头部欧拉角 ×3 + 双眼宽/高/位置/角度 + 间距 + 透视），25 个表情是一张 25×11 的参数表
  （`expressions.ts`，索引 0-24 语义与 v1 完全同源——POOLS 引用不变）。v1 的 `expressions.json`（烘焙 48 点轮廓）已删除。
...
| `geometry.ts` | 3D 姿态与投影渲染：四元数、`poseFromExpression`、`renderAvatar(pose, surface, blink)` → 头/背层/双眼 path；sphere/mickey/capsule 走椭球解析投影，cube/diamond 顶点凸包，cylinder/cone 采样凸包（模块级 cache）；`Expression` 类型 + `expressionFields` + `nearestEquivalentAngle` |
| `expressions.ts` | 25 表情参数表（v1 索引语义不变）+ `NEUTRAL_EXPRESSION`；**改一行 = 改所有引用该索引的状态的脸** |
...
| `shapes.ts` | 形状词表 = 8 种曲面原语（sphere/capsule/cylinder/cone/cube/diamond/mickey/cursor）→ `SurfaceConfig`；`LEGACY_BOT_SHAPE_MAP`（v1 8 形双射换代）；`BACK_PATH_COUNT`（mickey 双耳/cursor 锥体背层槽位）；`BOT_AVATAR_SHAPES` 是 parity 闸抽取锚点；viewBox `-150 -150 300 300` |
...
| kind | 形状 | 语义 |
|---|---|---|
| `{type:'bot', shape, color}` | canonical | 编辑器保存的选择；shape 词表 = v2 8 原语，v1 8 形名经 `LEGACY_BOT_SHAPE_MAP` 读侧换脸（blob→sphere / squircle→cube / egg→cylinder / wedge→cone / hex→diamond / cloud→mickey / teardrop→cursor / capsule→capsule），存量行不迁移不回写 |
