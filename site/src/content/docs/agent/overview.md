...
`mailagent` 是 MailAgent 后端的**单入口命令行工具**，专为 CLI / 自动化脚本 / AI agent 调用而设计。它把散落在 `scripts/*.py` 里的核心能力（邮件 CRUD、全文搜索、LLM 分类、Notion 重传、健康检查……）整合成一套 `<noun> <verb> [<id>] [flags]` 的命令树，风格与 `gh` / `kubectl` / `aws-cli` 一致。
...
- **16 个命令组**：`email` / `attachment` / `llm` / `kos` / `notion` / `calendar` / `debug` / `backfill` / `project-progress` / `init` / `folder` / `report` / `api-key` / `im` / `contact` / `admin`。另有 `serve` / `serve-api` 两个长驻服务命令（无子命令，不产出 `-o json` 结构化输出）。
...
| 组 | 职责 | 读/写为主 |
|---|---|---|
| `email` | 邮件 CRUD + 搜索 + 重传 + flag + 置顶 + 归档 + 草稿 + 发送 + 退订 | 读 + 写 |
| `attachment` | 附件 list / download / search / extract（文本抽取）/ cleanup-orphans | 读 + 写 |
| `llm` | LLM 分类填 AI 字段 + selftest + stats + 路径对比 | 读 + 写 |
| `kos` | KOS 入库台账统计（status 分布 / 错误码 / 重试积压 / 健康） | 读 |
| `notion` | Notion 直接操作（update-flag / orphan / archive / create-task） | 写 |
| `calendar` | 会议展开 / occurrence 查询 / RSVP / CalDAV 事件 CRUD / recurring discover-replay | 读 + 写 |
| `debug` | raw MIME / mail-structure / inline-images / notion-page | 读 |
| `backfill` | 历史正文回填 + metadata 回填 | 写（长任务） |
| `project-progress` | 项目周报同步外挂（xlsx → Notion） | 写 |
| `init` | 初始化同步 7 个 sub-action | 写（长任务） |
| `folder` | 存档 / 草稿文件夹 discover / enable / disable / create / rename / delete / cleanup（davmail-only） | 读 + 写 |
| `report` | 报告 Agent run / list / get / delete / config-get / config-set / agent-create / agent-delete | 读 + 写 |
| `api-key` | scoped Bearer agent key：create / list / revoke / rotate | 读 + 写 |
| `im` | 飞书对话绑定：pair 出一次性绑定码 / status | 读 + 写 |
| `contact` | 通讯录 backfill 扫描催跑 + 聚合校准 | 写 |
| `admin` | 统计 / 健康 / db-version / 死信 / cleanup / repair / config 读写 | 读 + 写 |

完整动作 + flag + `jq` 样例见 [命令组参考](/agent/commands/)。
...
mailagent --help       # 列全部命令组 + 全局 flags
```
...
下面这段脚本演示一次**读 + 写完整流程**：健康检查 → 搜索 → 读一封正文 → 改一个 flag，每一步都做退出码处理。它是写自动化的起手式，建议照着改。
...
- [命令组参考](/agent/commands/) — 每组 synopsis + jq 样例
