...
有时你不知道该用哪个关键词，只记得大概想找什么——这时可以让 AI 替你搜。在 `⌘K` 里直接输入一句自然语言（比如"上周 Alice 发来的那封关于产品评审的邮件"），命令面板顶部会出现一行 **「AI 理解」**，回车即可。

AI 会把你的话翻译成精确的搜索条件、必要时多轮缩小或放宽范围，最后把真正相关的邮件挑出来。它**只会从真实命中的邮件里选**，不会编造不存在的邮件。

> 人工搜索和 AI 搜索底层用的是**同一套搜索引擎**——所以同一个问题，无论你手敲条件还是交给 AI，命中的邮件都是一致的。

## 更顺手的小功能

- **未读 / 有附件 快捷过滤**：在 `⌘K` 里输入关键词后，下方会出现「未读」「有附件」小标签，点一下就把对应条件加进查询，再点一下取消。
- **最近搜索**：刚打开 `⌘K`（还没输入时）会列出你最近搜过的词，点一下直接重搜。
- **收藏搜索**：常用的复杂查询可以「收藏此搜索」，下次从收藏列表一键调出。
