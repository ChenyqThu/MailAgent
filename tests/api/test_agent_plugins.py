...
def test_export_whitelist_and_avatar_shapes(plugin_client):
    ...
    store.update_agent("custom-bot", wire.config_patch_to_db({"avatar": {"type": "bot", "shape": "teardrop", "color": "teal"}}))
    assert client.get("/api/report-agents/custom-bot/export").json()["data"]["agent"]["avatar"] == {"type": "bot", "shape": "teardrop", "color": "teal"}
