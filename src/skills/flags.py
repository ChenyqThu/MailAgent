...
AGENT_PLUGINS_ENV = "MAILAGENT_AGENT_PLUGINS"
...
def agent_plugins_enabled() -> bool:
    """Return the current Agent Plugins flag; absent or malformed values default on."""
    try:
        from dotenv import dotenv_values

        from src.api.deps import get_env_file_path

        raw = (dotenv_values(get_env_file_path()) or {}).get(AGENT_PLUGINS_ENV)
    except Exception:  # noqa: BLE001
        raw = None
    if raw is None or raw == "":
        return True
