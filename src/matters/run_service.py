...
#:
#: S3（08-18）加了背景与目标、完成标志；v61（08-19）把前者拆成 `background` / `goal`
#: 两项。owner 的裁决是「main agent（本人在场）直写 + 审批卡；跟进 Agent 只能提案」，
#: 所以这三个字段在这里出现，但**不进** headless run 的直写路径（matter_followup 的
#: 矩阵行本来就拒 domain_write）。
#:
#: 🔴 改这个元组必须同步 `proposal_scope.PROPOSAL_TOUCHABLE_FIELDS` —— 那里是提案失效
#: （stale）判据的目标集。漏加 = owner 一边手改背景 / 目标、Agent 的旧提案一边还带着旧
#: 文案等着 accept，accept 时**静默覆盖** owner 刚写的新值。有闸盯着两者一致。
