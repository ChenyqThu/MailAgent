    ...
    "input_schema": {
        ...
        "properties": {
            "confidence": {
                ...
                "description": (
                    "分类置信度 0.0-1.0。"
                    "默认阈值 0.4（可通过 LLM_LOW_CONFIDENCE_THRESHOLD 调整）："
                    "低于此值才保留 '未处理' 留给人工复核。"
                    "评分基准：清楚明确的邮件（系统通知、明显的请求邮件）≥ 0.8；"
                    "判断方向明确但细分类型有点犹豫 0.5-0.7；"
                    "完全不知道怎么分才给 < 0.4。"
                    "**遇到不确定时倾向给 0.5+，让邮件进入 AI Reviewed 流程**——"
                    "Kevin 倾向于多看，宁可标得偏高也别漏。"
                ),
            },
            "ai_summary": {
                "type": "string",
