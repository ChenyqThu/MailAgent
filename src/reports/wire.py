...
# 0813 成品目录化（owner 拍板照搬 avatar-lab 成品库、自编形状全退役）：9 个成品形状
# 对应 lab defaultStudioDocument 的 10 个 avatar（Grok bot 与 Strobi 在 lab 源数据里
# 仅 depth 差 0.0367、视觉不可分辨，owner 拍板合并 → sphere；其余 Nova→capsule /
# Citrus→cone / Cubee→cube + freddy/sunee/kirby/cloudee/onee，后五者除 onee 外带
# 组合身体）。组合身体是形状名在 TS 侧的派生数据（shapes.ts SHAPES[shape].nodes），
# wire 结构不变仍是 {type,shape,color}。
# v1 8 形（blob/squircle/egg/wedge/hex/cloud/teardrop/capsule）、v2 退役 4 形
# （cylinder/diamond/mickey/cursor —— 无 lab 成品对应物的自编原语）与 strobi
# （短暂在词表一批后并入 sphere）由前端 shapes.ts LEGACY_BOT_SHAPE_MAP 读侧换脸，
# 写侧（本白名单 + 编辑器）只认现词表，存量行不迁移。
BOT_AVATAR_SHAPES = (
    "sphere",
    "capsule",
    "cone",
