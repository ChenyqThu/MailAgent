...
2. 每条建议必须带至少一条邮件证据（message_id + 一句引文），无证据不提。
3. 已锁定字段不再提建议，除非新证据晚于锁定时间且与现值直接矛盾。
4. 换邮箱先找交接句，再比较签名档与同线程接续；成对建议哪个做主邮箱、哪个旧地址标曾用。
5. 不确定就写入 contradictions，不猜；不删数据；隐藏与标曾用邮箱都必须保持可逆。"""
