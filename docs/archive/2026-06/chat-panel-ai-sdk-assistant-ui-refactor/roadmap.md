...
- **Phase 04a A2UI 富工具卡片 + edit→re-approve ✅**（2026-06-24，commit `09424fd4`，全程 flag-off `MAILAGENT_A2UI_TOOL_CARDS`）：A2UI ComponentRegistry（`byName`→assistant-ui `tools.by_name`/miss→generic ToolTraceCard 不阻断）+ 富卡片（DraftReplyCard[edit 可编辑]/NotionSyncCard/通用审批卡）+ `buildToolA2UIPayload` 单一真源。**核心 = 域内 re-approve 侧信道**（解 03b 的 edit→re-sign gap）：ai@6 验签绑 history input + `signToolApproval` 未导出 → 编辑**从不进 ai@6 history**，卡片改正文 → `POST /api/ai/approval/resolve` → `ApprovalGuard.applyEdit` 存 `editedInput`（仅 editableFields，identity pin）→ execute 跑编辑后 input（secret 保持 on，无安全回退）。a2ui 只进审计（`chat_tool_call.ui_payload_json`，`CHAT_DB_VERSION 10→11`），不进模型 result（保 03b parity）。R5 零改 rules.py。验收：typecheck 0 · vitest **1828** · agent_eval **88** · 卡片截图 dark+light · reviewer(opus) APPROVE（6 不变式）。落地见 [architecture §13.11](./architecture.md#1311-phase-04a-落地2026-06-24a2ui-componentregistry--富工具卡片--editre-approve)。
- **Phase 04b 高风险外发 email_prepare_send + SendApprovalCard + 双 guard ✅**（2026-06-25，commit `66d1b489`，全程 flag-off `MAILAGENT_AI_SDK_SEND_TOOL`）：唯一真发信工具（blocking tier，needsApproval 恒 true，**工具名刻意不叫 email_send**）经 SendApprovalCard 人工确认 + **双 guard** 才走真实 SMTP。gateway（`security/{sendToken,approval}.ts` + `tools/send.ts`：verify + `consume` 一次性幂等 + content hash + HMAC token）↔ Python（`src/services/send_guard.py` + `POST /api/email/send-approved`：签名 + expiry + 重算 hash + `SendLedger.reserve` 在 send 前 fail-closed + MailWriteService.send）。**🔴 跨语言 content hash** = 行分隔规范串（**刻意非 JSON**）TS↔Python golden `f203073…` 两侧断言。risk 双词汇 blocking↔edit（catalog/CHECK 只认 silent/preview/edit）。chat_tool_call + content_hash/idempotency_key → `CHAT_DB_VERSION 11→12`；catalog 加 `email_prepare_send`（`gateway_only:true`，validate_catalog 豁免但 legacy 仍严格）。验收：typecheck 0 · vitest **1856** · agent_eval **89** · `run_baseline --compare` **29==29** · rules.py 零改 · 真发自测信 dogfood 落 Sent + replay 拒 · 卡片截图 dark+light · reviewer(opus) APPROVE（9 不变式）。落地见 [architecture §13.12](./architecture.md#1312-phase-04b-落地2026-06-25高风险外发-email_prepare_send--sendapprovalcard--双-guard)。
- **Phase 05 AG-UI Interop mirror ✅**（2026-06-25，commit `7ddd09a0`，全程 flag-off `MAILAGENT_AG_UI_MIRROR`）：旁路端点 `POST /api/ai/agui/chat` 把已稳定的 AI SDK UIMessage/tool/approval/context 映射成标准 **AG-UI event 流**（`events/eventMapper/interruptMapper/stateSnapshot/aguiRoute`），**复用与 /api/ai/chat 完全相同的 streamText + tools + 04b 双 guard approval、只换输出编码器**（抽 `chatRun.ts`/`httpUtil.ts` 单源）。🔴 无静默外发路径（resume 桥只翻 tool part 状态保签名·不动 signed input→二调仍经同一 guard，未知/malformed fail-closed）+ STATE_SNAPSHOT 脱敏（token/secret 键名任意深度 drop·大正文截断）+ rules.py 零改（旁路不产 legacy trace）。验收：typecheck 0 · vitest **1886** · agent_eval **89** · eslint 0 · reviewer(opus) APPROVE（4 安全不变式全 PASS）。assistant-ui AG-UI runtime smoke 用 Gateway 侧 route SSE golden 替代（`@ag-ui/*` 不入依赖树）。落地见 [architecture §13.13](./architecture.md#1313-phase-05-落地2026-06-25ag-ui-interop-mirror)。
- **AI SDK 生产 parity ✅（2026-06-25，commit `b758d25f`，flag-off `MAILAGENT_AI_SDK_CONTEXT_INJECTION`）**：AI SDK chat 路径补齐 standing-context + 当前邮件上下文 + 会话重载，不再 context-light。① `ai-gateway/systemPrompt.ts` **复用 legacy `buildStableSystemPrompt(ctx=null)`**（同一 `/chat/config` 源 + 同一装配，`PRODUCT_SAFETY_FLOOR` 不可弱化，parity byte-identical 有测）+ typed `AgentContextSnapshot`（`shared/assistant/context/`，§6 budget 截断 + §7 `UNTRUSTED_*` 围栏）→ `buildContextSystemBlock` append；② `useChatRuntime({messages})` 会话重载（`session_id` 一致性判 ready 避 selectSession 竞态）；③ ContextChips 同源；④ wrapper TTL 缓存 provider fetch `/chat/config`（失败降级 context-light）。🔴 防注入纵深三处脱敏（围栏正文 / `<mailagent_context_json>` JSON 元数据块[攻击者可控 Subject/From，reviewer 抓的 HIGH] / START-line attrs）。验收：typecheck 0 · vitest **154 files/1913 passed** · agent_eval **89**（flag-gated → ≥ baseline）· reviewer(opus) APPROVE（修 1 HIGH+1 MEDIUM+2 LOW，复核无残留）。落地见 [architecture §13.14](./architecture.md#1314-phase-06-parity-落地2026-06-25context-injection--standing-context--会话重载)。
- **下一步 = 06 Cutover & Cleanup（epic 最后一阶）**：新会话默认切 assistant-ui + AI SDK Gateway + 下线 legacy chat UI 主路径（保 `MAILAGENT_CHAT_RUNTIME=legacy` 一键回滚 + 旧会话兼容读取 + backfill ui_message_json + resolve/agui/send/chat 端点 loopback CORS/Origin 收紧同批）。两步走：06a 切默认 + 移除 legacy UI 主路径；06b = AI SDK 新会话连续 dogfood 7 天无 P0/P1 后归档 harness。可粘贴 /goal = [goal-prompts.md「P4-Phase06」](../agent-experience-epic/goal-prompts.md)。

执行顺序（实际落地序）：`00 → 01 → 02 → 03a → 03b → 04a → 04b → 05 → AI SDK 生产 parity ✅ → 06 cutover`；每个 phase 验收叠加 `tests/agent_eval` baseline 不回退闸（§2 原则 6）。