...
### 任务 A2 — 灵动岛 harness 审批 UX 打磨（v1.2.0 dogfood 遗留；🔴 先修 [#36](https://github.com/ChenyqThu/MailAgent/issues/36) 面板 live-refresh 两轮未修复——最强假设 = renderer ai@6 部件形态与守卫谓词(ai@7 形态)不匹配致降级裸 isRunning 挡死，CDP 抓真形态再写测试）
