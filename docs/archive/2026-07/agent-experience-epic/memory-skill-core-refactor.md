...
- **M4b `update_system_md` 工具** ✅ **已落地**（core `c2da61e0` + 卡片 `db665ec6` + review 修复 `d4bf7f53`/`01f832ad`）：edit-tier 写，agent 提议覆写 soul/agent/rules/user 全文 → SystemDocApprovalCard 显**完整内容** approve/reject（soul/agent/rules 高危红样式 + floor 提示）。Python 端全现成（`set_profile_doc` + rules 经 `validate_rules_content`，NFKC+去零宽归一化闭 fullwidth/零宽绕过）。edit-tier 天然恒人审（auto-reversible 也不跳）；`PRODUCT_SAFETY_FLOOR` 是 code-prepend 结构上不可弱化。**审批 guard 收紧（codex HIGH-1）**：`approval.verify()` 对**所有 tier** 拒绝 raw-changed exec input（唯一合法变更=applyEdit override，identity 钉死）→ doc_name 不可被 retarget；这是跨切面修复，护住全部 edit/blocking 工具。
- **M4c `discover_skills` + `set_skill_enabled` 工具** ✅ **已落地**（同上 commits）：discover_skills（silent 读）列能力 + enabled/available/unavailableReason；set_skill_enabled（preview 写）启用/停用挂载，返回 mounted/available（**enable ≠ mount**，仅 enabled&&available 才 advertise 工具）。被挂载工具仍各自审批 → 启用非静默授权。3 工具进 `CORE_UNGATED`（永不 skill-gated），catalog 标 gateway_only。
