...
> **配套**：架构 [`docs/claude/service-layer-architecture.md`](./claude/service-layer-architecture.md) · 看板 [`docs/backend-service-migration-matrix.md`](./backend-service-migration-matrix.md)。
...
全部 cell ✅ + dead_letter/gt_30m 无异常 → 在看板 [`docs/backend-service-migration-matrix.md`](./backend-service-migration-matrix.md) 的「最终验收 gate」勾上 e2e/监控两项，重构正式收官归档。
