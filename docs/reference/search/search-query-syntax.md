...
- **批次3 PR-E（端点/工具面接 trigram）**：独立附件检索端点 `/api/attachment/search`（gateway 工具 `email_search_attachments` / CLI `attachment search` / skill `attachment_search` / webhook 事件同源，内核 `EmailRepository.search_attachment_texts`）此前**只查 `email_attachment_fts`（unicode61，仅正文）**，与上面 DSL 附件字段割裂——agent 经该工具搜中文子串/文件名是盲区。PR-E 起**路由内化**（`search_attachment_texts(query, *, raw)`，镜像 `search_email_bodies_with_meta` 的 raw+模式进函数模式）：`raw=False`（smart）时内核收原始 query，`trigram_enabled` 则 `query.split()` 各 term 过 `_route_text_term` → MATCH 型合成单 `email_attachment_fts_trigram` MATCH（跨 filename+text_content 两列，bm25+snippet）、2 字 CJK / 短拉丁 <3 走 `(filename OR text_content) LIKE`（rank null、按日期倒序、snippet fallback 前 80 字符）、1 字 CJK 丢弃；`raw=True`（用户手写 FTS5）直通老 unicode61 逐字节不变；flag off / v38 缺表 → 内核内 `smart_query_transform` + 老 unicode61 降级（**案 B 有命中**，与批次2 `attachment:` DSL 谓词的 fail-closed 空显式区分——端点是独立检索端点、老表恒可查）。粒度 = **附件级**（同邮件多附件各自出现，不复用融合 lane 的 email 级去重）。原 6 直调点删各自的 pre-transform、传原始 q + raw 标志（`search_attachment_texts_smart` wrapper 内化，reports 零改动）。pin 在 `TestSearchAttachmentTextsTrigram` + `tests/api/test_attachment.py`。
