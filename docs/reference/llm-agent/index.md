...
| [`im-feishu-chat.md`](./im-feishu-chat.md) | 动 `src/im/` / `/api/im/*` / gateway `im-chat` 入口 / 设置-AI「飞书对话」区前 | **飞书对话**（agent 的第四个场地）：lark-oapi 长连接挂 serve + `im_chat` 工具矩阵（读免批 / domain 写恒 HITL / connector 写 08-05 起按 per-tool 三档 / exec·capability_change·outbound 不注册）+ 飞书内按钮审批闭环（含 `repaused` 非终态陷阱）+ 会话映射与桌面可见 + `im:feishu` 凭证与绑定码 + 与通知 bot 的隔离边界 + 总闸 `MAILAGENT_IM_FEISHU`（默认开，cutover 2026-08-04） |
