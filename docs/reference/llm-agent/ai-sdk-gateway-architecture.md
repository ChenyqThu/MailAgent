...
- **稳定身份（SyncStore DB v42）**：`report_agent.avatar_json` 保存 Oreo `shape/palette/variant`；NULL 时由 agent id 确定性派生默认，不因重启变化。Settings 支持换一换/手选，AgentCard、会话/Chats 分组、run 历史、custom report 分组等三处以上统一消费 `AgentAvatar`；通知面仍可降级 initials。
