...
- **持久化**（B.4：复用 `ai_chat_messages`）：一条 `role='system'` 行——`content`=摘要 markdown、`metadata`=A.6 `CompactMessageMetadata`（kind/version/compactedThroughMessageId/firstKeptMessageId/tokensBefore/estimatedTokensAfter/model/reason/valid/createdAt）、`ui_message_json`=Compact 卡 UIMessage（`role:'system'` + 单 `data-compact` part 携带 metadata+summary）。这是仓内**首个** system 行写入路径。🔴 assistant-ui 要求 system 消息**恰好一段文本**，canonical 原样交给运行时会在渲染前抛「System messages must have exactly one text message part」、整个面板崩（09-10 自动压缩后实测）；所以回放层 `uiMessage.ts::chatMessageToUIMessage` 把它规整成「一段摘要文本」，顶层 metadata 原样保留（compactSelect 靠它认标记），另复制到 `metadata.custom.compact`（assistant-ui 的 system 消息只留 custom）供 `SystemMessage → CompactCard` 渲染；库里 canonical 不改写。回归闸 `tests/shared/assistant/compact_marker_reload.test.tsx` 真挂 runtime。
