...
- **下一步 = Phase 05 AG-UI Interop mirror**（旁路，gated `MAILAGENT_AG_UI_MIRROR`，goal-prompts P4-Phase05）：`/api/ai/agui/chat` 把 AI SDK UIMessage/tool/approval/context → 标准 AG-UI event；复用同一 streamText + 04b 双 guard。🔴 **AG-UI 不解锁 cutover**：cutover（06）真前置 = standing-context 注入（AI SDK 路径当前 context-light，无 soul/rules/memory/邮件上下文）+ 会话重载接线，留 05 之后的「AI SDK 生产 parity」phase。
