...
- **下一步 = Phase 03b（write tools preview + approval）**：email_flag/archive/draft_reply/sync_to_notion preview（gated `MAILAGENT_AI_SDK_WRITE_TOOLS` + needsApproval），approval 两次调用语义 + eval R5 recorder 重对齐（architecture §13.4，rules 零改）。
