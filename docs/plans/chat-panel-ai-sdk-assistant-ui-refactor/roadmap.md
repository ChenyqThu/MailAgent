...
- **下一步 = 〔AI SDK 生产 parity〕phase（cutover 06 真前置，非最后一阶）**，gated（默认 off → context-light 现状字节级不变）：🔴 **AG-UI 05 不解锁 cutover**——核实 AI SDK chat 路径目前 **context-light**：① standing-context 未注入（SOUL/AGENT/RULES/USER + memory_summary + skill 能力 + 当前邮件/anchor 上下文只在 legacy `platform.ts` 组装，`useMailAgentAiSdkRuntime` transport 只发 `{sessionId,model}`，`chatRun.ts` 的 `system` 无人填）；② 会话重载未接线（prior `ui_message_json` 未喂 `useChatRuntime({messages})`，§13.8.5）。这两项 + prompt-injection 防护 + body cap + remote-web CORS = cutover 前置（phase-06 §2）。spec = [context-injection.md](./context-injection.md)；可粘贴 /goal = [goal-prompts.md「P4-Phase-Parity」](../agent-experience-epic/goal-prompts.md)。**之后才是 06 cutover（切默认 runtime + 删 legacy UI）= epic 最后一阶**。

执行顺序（实际落地序）：`00 → 01 → 02 → 03a → 03b → 04a → 04b → 05 → 〔AI SDK 生产 parity〕 → 06`；每个 phase 验收叠加 `tests/agent_eval` baseline 不回退闸（§2 原则 6）。