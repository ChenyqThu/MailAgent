...
| 阶段 | Epic | 一句话 | 量级 | 文档 |
|---|---|---|---|---|
| **Now（≤2 周）** | E0 安全网 | CI 测试闸 + DB 备份/quick_check + 迁移守卫 + promote workflow + 依赖锁定 | ~1 周 | [e0](./e0-safety-net.md) |
...
| **Next（1-2 月）** | E2 减法 Sprint | 删 legacy harness（前置抽 prompt 层）+ outbox 收口删死分支 + fork CLI 退役 + 反向写归一；预期 -12~15k LOC | ~2 周 | [e2](./e2-subtraction-sprint.md) |
