...
| **P4** | 换引擎：assistant-ui + AI SDK Gateway，**每 phase 跑通 eval baseline 不回退** | [`../chat-panel-ai-sdk-assistant-ui-refactor/`](../chat-panel-ai-sdk-assistant-ui-refactor/) | ◐ **Phase 00 spike ✅ GO** + **Phase 01 Shell ✅**（`b82ee24e`）+ **Phase 02 AI SDK Gateway ✅**（`a6d189ac`）+ **Phase 03a read tools ✅**（2026-06-24，flag-off，9 read 工具 + DomainClient + parity 钉死、vitest 1756、eval 85≥baseline）；**Phase 03b write tools + approval 待开工** |
...
| [`goal-prompts.md`](./goal-prompts.md) | **P1-P4 可粘贴 `/goal` session 启动手册**（下一 session 从这里取 **P4-Phase03b write tools + approval**） |
