...
00 Spike ✅ → 01 Shell ✅ → 02 Gateway ✅ → 03a read ✅ → 03b write ✅ → 04a A2UI cards ✅
  ...
  → 〔AI SDK 生产 parity：standing-context 注入 + 会话重载 = cutover 真前置，**← 下一个 phase**〕→ 06 cutover
```
> 🔴 05（AG-UI）是互操作旁路、**不解锁 cutover**；06 切流前必须先补「AI SDK 生产 parity」（AI SDK 路径当前 context-light，见进度表 P4 注 + phase-06 §2）。**05 已完成 → 下一个 phase = AI SDK 生产 parity（goal-prompts.md「P4-Phase-Parity」），它才是 cutover 06 的真前置；不是最后一个 phase。**
...
- [ ] **🔴 cutover 前置 = AI SDK 生产 parity phase（← 下一个 phase；05 AG-UI 旁路不解锁）**：standing-context 注入（SOUL/AGENT/RULES/USER + memory_summary + skill 能力 + 邮件/anchor 上下文 → AI SDK 路径，当前 context-light）+ 会话重载接线（prior `ui_message_json` → `useChatRuntime({messages})`，§13.8.5）+ prompt-injection 防护（untrusted-content markers）+ body cap 提升 + remote-web CORS 收紧。spec=chat-panel `context-injection.md`；可粘贴 /goal=goal-prompts.md「P4-Phase-Parity」。
