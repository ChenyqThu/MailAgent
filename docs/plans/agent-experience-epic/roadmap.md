...
00 Spike ✅ → 01 Shell ✅ → 02 Gateway ✅ → 03a read ✅ → 03b write ✅ → 04a A2UI cards ✅
  → 04b high-risk send ✅ → 05 AG-UI mirror ✅（旁路，`7ddd09a0`）
  → 06-parity 生产 parity ✅（standing-context 注入 + 会话重载，`b758d25f`）
  → 06a cutover A–G ✅（chat_db v13 / master flag / 急切会话 / per-session 路由 / 健康降级 / CORS / backfill，全 dark）
  → 06 H = flip 默认（**gate 在 dogfood，← 下一步**）→ 06b 7 天观察窗 → 删 legacy harness
```
> 🔴 05（AG-UI）是互操作旁路、**不解锁 cutover**。**06-parity（AI SDK 生产 parity）✅ + 06a cutover A–G ✅ 均已落地（全程 flag/master 默认 OFF→dark）**；剩 **06 的 H = 一行式翻 master 默认 `?? '' → '1'`，gate 在 electron+web dogfood**（详见 goal-prompts.md「P4-Phase06b」+ architecture §13.15.3）。H 翻后才进 **06b**（连续 dogfood 7 天无 P0/P1 → 归档 legacy harness 主路径）。**一键回滚铁律**：`MAILAGENT_CHAT_RUNTIME=legacy` 随时把默认切回 legacy。
...
- [x] **AI SDK 生产 parity（06-parity）✅**（2026-06-25，`b758d25f`）：standing-context 注入（复用 legacy `buildStableSystemPrompt`）+ 会话重载（`ui_message_json` → `useChatRuntime({messages})`）+ prompt-injection 四处脱敏 + body cap 8MiB（architecture §13.14）。
- [x] **旧会话可读 + Gateway 降级（06a）✅**：per-session backend_kind 路由（旧 custom-api→legacy / notion-agent→只读）+ /health 探针不可用自动降级 legacy + 非阻断提示（Chunk D/E，`1a5e9670`/`ac085b81`）。
- [ ] **06 H flip + dogfood（← 下一步）**：翻 master 默认 `?? '' → '1'`（电桌+web 两 config）→ electron+web 各 dogfood 回归场景 → 连续 7 天无 P0/P1（06b）才归档 legacy harness 主路径（chat-panel phase-06 §8 + architecture §13.15.3）。
